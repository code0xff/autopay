...
> **클릭 전 준비 확인 (2026-09-26, 실사용 버그)**: 쿠팡 결제창은 React 앱이라
> 서버 렌더된 [결제하기]가 **먼저 보이고 하이드레이션이 끝나야 `onClick`이
> 붙는다**. 그 전에 누르면 오류도 없이 **아무 일도 일어나지 않고** 완료 신호만
> 기다리다 타임아웃난다(실제로 3회 연속 발생). 버튼은 찾아지고 `disabled`도
> 아니어서 증상만으로는 원인을 알 수 없었다 — 라이브 DOM에서 핸들러가
> `__reactProps$*.onClick`으로 붙는 것을 확인해 특정했다.
> - **왜 이제야 터졌나**: 확인(confirm) 게이트가 있을 땐 사용자가 승인하는 수십
>   초 사이에 하이드레이션이 끝나 우연히 동작했다. 정책에서 확인을 끄자
>   (§2.5 2026-09-23 변경) 체크아웃 로드 직후 클릭이 되면서 매번 실패했다.
>   즉 **사람의 지연에 의존하고 있던 것**이 드러난 셈이다.
> - **대응**: `PageBridge.waitClickable`로 핸들러가 붙을 때까지 대기한 뒤
>   **첫 클릭을 한 번만** 한다(최대 15s). 넘으면 클릭하지 않고
>   `failed(pay_button_not_ready)`로 끝낸다 — 눌러보고 재시도하는 방식은
>   중복 결제 위험이 있어 채택하지 않는다(§9 교차 워커 원자성과 같은 이유).
> - **⚠️ 이 검사만 MAIN 월드에서 돌린다.** `chrome.scripting.executeScript`는
>   기본이 **격리 월드**인데, `__reactProps$*`·`onclick` 같은 expando는 페이지
>   월드의 것이라 **격리 월드에서는 보이지 않는다**. 처음엔 격리 월드에서
>   검사했다가 항상 false가 나와 결제가 전부 `pay_button_not_ready`로 실패했다.
>   반대로 `.click()`은 DOM 이벤트라 월드를 넘어 전달되므로 **클릭은 격리
>   월드에서 그대로** 한다(페이지 스크립트와 섞이지 않게). MAIN 주입이 거부되면
>   결제를 막지 않고 `readyState=complete` + 정착 시간으로 폴백한다.
> - 회귀 테스트: `adapters.test.ts` "버튼이 눌리는 상태가…"·"끝내 눌리는 상태가…".

```
verify(tabId):
  - 쿠팡 인-페이지 체크아웃에서 결제 예정 금액 DOM 파싱 → amount
pay(input):
  0. [결제하기]에 핸들러가 붙을 때까지 대기(하이드레이션) — 안 되면 클릭 없이 실패
...
awaitCompletion 폴링 중 passwordUi(보이는 요소) 최초 감지:
  1. onPasswordRequired() 1회 호출 → 브로커가 notify("enter_password_on_page")
     ...
     · 알림은 requireInteraction(자동 소멸 금지 — 놓치면 타임아웃으로 끝난다)
     · 알림 클릭 → 그 결제 탭을 활성화·포커스(탭을 찾아 헤매지 않게)
...
**알림 클릭 → 결제 탭 포커스 (2026-09-25)**
탭이 수십 개면 결제창을 찾는 데만 시간이 걸려 10분을 넘길 수 있다. 그래서 비번
알림을 클릭하면 해당 결제 탭을 `chrome.tabs.update({active:true})` +
`chrome.windows.update({focused:true})`로 바로 띄운다.
- 탭 id는 **알림 id에 인코딩**한다(`autopay-pw-<tabId>`). 핸드오프는 최대 10분이라
  클릭 처리가 알림을 띄운 것과 **다른 SW 인스턴스**에서 일어날 수 있는데, id에
  담으면 영속 저장 없이 동작한다. 복원은 fail-closed(접두사·숫자 형식이 정확히
  맞을 때만 — `Number("")===0` 때문에 빈 접미사가 탭 0을 가리키던 버그를 테스트로
  잡았다). 회귀 테스트: `chrome-notify.test.ts`.
- `onClicked` 리스너는 background 최상위에 동기 등록한다(MV3는 SW 재기동 시 동기
  등록된 리스너만 복원).
- **왜 링크가 아니라 알림인가**: "채팅창에 결제 탭 링크를 출력해 클릭"은 (a) 새
  탭이 열려 브로커가 폴링하는 `checkoutTabId`와 달라져 "쿠팡은 결제됨 / 기록은
  failed" 불일치를 오히려 만들고, (b) *신뢰하지 않는* 에이전트가 비번 입력 화면
  링크를 제시하는 습관을 들여 프롬프트 인젝션 시 유사 도메인 피싱과 구별할 수
  없게 된다. 사용자에게 말을 거는 주체는 **신뢰 영역인 브로커**여야 한다(§5).
- 사이드패널에 같은 기능의 버튼은 두지 않았다 — 알림 클릭과 탭 직접 클릭으로
  이미 두 경로가 있어 중복이다.
