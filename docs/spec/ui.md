...
3. ~~**본인 식별 정보 입력**~~ — 2026-09-23 UI에서 제거(패턴 B 카카오·토스
   어댑터가 아직 placeholder라 입력할 곳이 없음). refstore 저장 로직은 남겨 두고,
   패턴 B 라이브 착수 시 설정 탭에 다시 붙인다(`spec/refstore.md`).
