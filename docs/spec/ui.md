...
| 탭 | 내용 |
|---|---|
| **홈** | 시작하기 체크리스트(확인 가능한 단계가 끝나면 숨김) · **승인 카드**(대기 건, 없으면 "승인 대기 중인 결제가 없습니다") · 잠금 해제(잠겨 있고 카카오·토스 대기 시에만) · **오늘 남은 한도**(일 잔여 mono·잔여 횟수·월 잔여·건당 최대·승인 규칙). 대기 건이 있으면 탭에 개수 배지(warn) |
| **정책** | `PolicyForm` — 한도(건당/일/월/횟수)·확인 임계값·항상 확인·허용 머천트·결제수단·카테고리 |
| **기록** | 감사 로그 테이블 |
| **설정** | MCP 브리지(토큰·연결 상태) · 잠금 해제 · 본인 식별 정보(패턴 B PII) · 사이트 접근 고지 |
...
패스프레이즈 잠금은 **본인 식별 정보(패턴 B PII)의 암호화 키**일 뿐이다. 모든
탭은 잠금과 무관하게 보인다. 설정 탭에서는 본인 식별 정보 카드 바로 위에 해제
카드를 두고, **홈 탭에서는 잠겨 있고 카카오·토스(패턴 B) 승인이 대기 중일 때만**
해제 카드를 띄운다 —
