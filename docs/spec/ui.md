...
- **제거(2026-09-25)**: 패턴 C 승인 카드의 "폰 승인이 없어 이 확인이 유일한
  게이트" 안내. **사실이 아니게 됐다** — 실측 결과 승인 직후 쿠팡 FDS가 6자리
  비번을 요구하는 경우가 흔해서(`docs/payment-flows.md §4`) 우리 확인이 마지막
  게이트가 아니다. 게다가 "승인하면 결제 완료"로 읽혀 기대를 거꾸로 설정했다
  (실제로는 비번을 입력하지 않으면 타임아웃). 비번이 필요한 순간의
  `enter_password_on_page` 통지가 더 정확한 시점 안내라 사전 경고는 불필요.
