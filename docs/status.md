...
> 갱신: 2026-09-26. 무엇이 **실제로 만들어져 테스트·빌드로 검증**됐고, 무엇이
> 남았는지 정직하게 기록한다.
...
| **결제 버튼 하이드레이션 대기** (2026-09-26) | ✅ | [결제하기]를 찾아 눌러도 화면이 전혀 안 변하고 타임아웃나던 실사용 버그 — React 하이드레이션 전 클릭은 조용히 무시된다. `waitClickable`로 핸들러가 붙은 뒤 **한 번만** 클릭(재시도는 중복 결제 위험이라 안 함), 안 되면 `failed(pay_button_not_ready)`. 회귀 테스트 2건. `executor.md §3.1` |
| **고아 실행 즉시 회수** (2026-09-26) | ✅ | 워커가 죽으면 영속 색인만 남아 리로드해도 페이지 도구가 계속 잠겼다(핸드오프 상한 10분 탓에 11분+알람 주기). 색인에 있는데 `inFlight`에 없으면 죽은 워커의 잔여이므로 시간과 무관하게 즉시 회수 + 기동 시 스윕. 시간 기준은 이 워커가 실제로 돌리는 건에만 적용. 회귀 테스트 #18·#18-b |
| Codex 리뷰 | ✅×4 | M0·M1코어·M1익스텐션·최종, High/Medium 반영 |

**테스트 총계**: 191 (shared 18 + broker-extension 151 + mcp-server 22). typecheck·biome 클린.
