...
export const COUPAY: AdapterConfig = {
  ...
  merchantName: "쿠팡",
  selectors: {
    // ⚠️ 2026-09-23 실사용 중 발견한 버그: "최종 결제 금액"은 값이 바로 붙은
    // 라벨이 아니라 **섹션 제목**이다. 그 뒤에 젤 먼저 나오는 "…원"은 "총 상품
    // 가격"(할인 전 금액, 예: 16,500원)이지 실제 결제 금액이 아니다. 진짜 최종
    // 금액은 그 섹션 맨 아래 "총 결제 금액" 옆(예: 16,300원, 와우 할인 반영 후).
    // 이걸 몰라서 "최종 결제 금액"을 앵커로 썼다가 "총 상품 가격"을 잘못 집어
    // amount_mismatch로 정상 거절되는 걸 실사용에서 확인했다(다행히 결제
